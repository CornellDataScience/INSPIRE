...
library(mclust)
setwd("../INSPIRE")
spore <- fread("recommendations_with_components.csv")
summary(spore)
head(spore)
nrow(spore)
n_distinct(spore$id)
...
k.max <- 15
wss <- sapply(1:k.max, 
              function(k){kmeans(numerical_spore[,c(3:8,10:12)], k, nstart=50,iter.max = 15 )$tot.withinss})
wss
plot(1:k.max, wss,
...
fviz_nbclust(spore[,c(3:8,10:12)], kmeans, method = "silhouette")

sp_km_clust = kmeans(spore[,c(3:8,10:12)], centers = 2, nstart = 100)
spore$cluster = sp_km_clust$cluster
#Distance matrix
...
write.csv(spore[,c(2:13)],"modeless_clustered.csv")
table(spore$cluster)
